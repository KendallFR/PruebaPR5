...
import {
  ArrowLeft, History, ShieldCheck, CheckCircle,
  Calendar, User, Tag, Sparkles, FilmIcon,
} from "lucide-react";
import { LoadingGrid } from '../ui/custom/LoadingGrid';
import { EmptyState } from '../ui/custom/EmptyState';
import { categoriaStyles, getGlowByCategoria } from '../../utils/categoriaColors';

/* ── Color dinámico por primera categoría ── */
function getGlowColor(categorias) {
  if (!categorias?.length) return { hex: "#3b82f6", rgb: "59,130,246" };
  const glow = getGlowByCategoria(categorias[0].descripcion);
  return { hex: glow.hex, rgb: glow.rgb };
}
...
export function DetailCarta() {
  ...
  const glow       = getGlowColor(categorias);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#020617] via-[#0a0f1e] to-[#020617] px-4 py-10">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-20 right-10 w-80 h-80 rounded-full blur-3xl opacity-5"
          style={{ background: glow.hex }} />
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="w-full lg:w-auto flex flex-col items-center gap-4">
            {c.imagenes?.length > 0 ? (
              <>
                <div className="relative w-64 h-[360px] rounded-[18px] overflow-hidden border-[3px] shadow-[0_20px_60px_rgba(0,0,0,0.8),inset_0_1px_0_rgba(255,255,255,0.20)] ring-1 ring-black/50 bg-[#0a0f1e] transition-all duration-500 hover:scale-[1.02]"
                  style={{
                    borderColor: `rgba(${glow.rgb}, 0.5)`,
                    boxShadow: `0 20px 60px rgba(0,0,0,0.8), 0 0 40px rgba(${glow.rgb}, 0.15), inset 0 1px 0 rgba(255,255,255,0.15)`,
                  }}>
                  <img src={`${BASE_URL}/${c.imagenes[0].imagen}`} alt={c.nombre} className="w-full h-full object-cover" />
                  ...
                  <div className="absolute inset-0 pointer-events-none"
                    style={{ background: `linear-gradient(135deg, rgba(${glow.rgb},0.08) 0%, transparent 50%, rgba(${glow.rgb},0.04) 100%)` }} />
                  ...
                  <div className="flex gap-3 flex-wrap justify-center">
                    {c.imagenes.slice(1).map((img, i) => (
                      <div key={img.idImagen}
                        className="relative w-20 h-28 rounded-[10px] overflow-hidden border-2 border-white/20 shadow-lg shadow-black/50 bg-[#0a0f1e] transition-all duration-200 hover:scale-110 hover:border-white/40 cursor-pointer">
                        <img src={`${BASE_URL}/${img.imagen}`} alt={`${c.nombre}-${i + 2}`} className="w-full h-full object-cover" />
              ...
              <div className="w-64 h-[360px] rounded-[18px] border-2 border-white/10 bg-[#0a0f1e] flex items-center justify-center text-white/15">
          ...
          <div className="flex-1 space-y-4">
            ...
            {/* Nombre + descripcion */}
            <div className="relative overflow-hidden rounded-3xl p-7 border border-white/[0.07] bg-[#0d1424]/90 backdrop-blur-xl"
              style={{ boxShadow: `0 0 40px rgba(${glow.rgb}, 0.05), inset 0 1px 0 rgba(255,255,255,0.05)` }}>
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-2xl flex items-center justify-center border"
                      style={{ background: `rgba(${glow.rgb}, 0.12)`, borderColor: `rgba(${glow.rgb}, 0.25)` }}>
                      <Sparkles className="w-5 h-5" style={{ color: glow.hex }} />
                    </div>
                    <h1 className="text-2xl font-black text-white tracking-tight">{c.nombre}</h1>
                  </div>
                  <span className={`px-2.5 py-1 rounded-xl text-[11px] font-bold border shrink-0 flex items-center gap-1.5 ${
                    isAvailable ? "bg-green-500/10 text-green-300 border-green-500/20" : "bg-red-500/10 text-red-300 border-red-500/20"
                  }`}>
                ...
                {c.descripcion && (
                  <p className="text-white/40 text-sm leading-relaxed pl-14">{c.descripcion}</p>
                )}
              </div>
            </div>
            ...
            {/* Categorías con colores para las 21 ── */}
            <div className="p-5 rounded-2xl bg-[#0d1424]/80 border border-white/[0.06] backdrop-blur-xl">
              ...
              <div className="flex flex-wrap gap-2">
                {categorias.length > 0 ? categorias.map((categoria) => {
                  const style = categoriaStyles[categoria.descripcion];
                  const glowCat = getGlowByCategoria(categoria.descripcion);
                  if (style) {
                    return (
                      <span key={categoria.idCategoria}
                        className={`px-3 py-1.5 rounded-full text-[11px] font-bold border transition-all duration-200 hover:scale-105 ${style}`}>
                        {categoria.descripcion}
                      </span>
                    );
                  }
                  // fallback dinámico para cualquier categoría sin mapeo
                  return (
                    <span key={categoria.idCategoria}
                      className="px-3 py-1.5 rounded-full text-[11px] font-bold border transition-all duration-200 hover:scale-105"
                      style={{
                        background: `rgba(${glowCat.rgb}, 0.10)`,
                        borderColor: `rgba(${glowCat.rgb}, 0.25)`,
                        color: glowCat.hex,
                      }}>
                      {categoria.descripcion}
                    </span>
                  ...
                  <span className="text-white/30 text-sm">Sin categorías</span>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-1">
              <Button onClick={() => navigate(`/carta/${id}/subastas`)}
                className="flex-1 rounded-2xl h-12 bg-gradient-to-r from-yellow-400 to-yellow-300 hover:from-yellow-300 hover:to-yellow-200 text-black font-bold text-sm shadow-lg shadow-yellow-400/25 hover:shadow-yellow-400/40 hover:scale-[1.02] transition-all duration-200 flex items-center justify-center gap-2">
                <History className="w-4 h-4" />
                Historial de Subastas
              </Button>
              <Button onClick={() => navigate(-1)}
                className="flex-1 rounded-2xl h-12 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] hover:border-white/15 text-white/50 hover:text-white/80 text-sm transition-all duration-200 flex items-center justify-center gap-2">
                <ArrowLeft className="w-4 h-4" />
                Regresar
              </Button>
            </div>
