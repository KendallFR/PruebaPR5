...
const getTypeStyles = (categorias) => {
  ...
  switch (tipo) {
    case "electrico":
      return { glow: "hover:shadow-yellow-400/80 hover:border-yellow-400/80", ring: "group-hover:ring-yellow-400/70", badge: "bg-yellow-400/20 text-yellow-300", gradient: "from-yellow-400/30 via-yellow-300/10 to-transparent" };
    case "fuego":
      return { glow: "hover:shadow-red-500/80 hover:border-red-500/80", ring: "group-hover:ring-red-500/70", badge: "bg-red-500/20 text-red-300", gradient: "from-red-500/30 via-red-400/10 to-transparent" };
    case "agua":
      return { glow: "hover:shadow-blue-500/80 hover:border-blue-500/80", ring: "group-hover:ring-blue-500/70", badge: "bg-blue-500/20 text-blue-300", gradient: "from-blue-500/30 via-blue-400/10 to-transparent" };
    case "pokemon":
      return { glow: "hover:shadow-purple-400/80 hover:border-purple-400/80", ring: "group-hover:ring-purple-400/70", badge: "bg-purple-400/20 text-purple-300", gradient: "from-purple-400/30 via-purple-300/10 to-transparent" };
    case "entrenador":
      return { glow: "hover:shadow-orange-400/80 hover:border-orange-400/80", ring: "group-hover:ring-orange-400/70", badge: "bg-orange-400/20 text-orange-300", gradient: "from-orange-400/30 via-orange-300/10 to-transparent" };
    case "objeto":
      return { glow: "hover:shadow-slate-300/40 hover:border-slate-300/40", ring: "group-hover:ring-slate-300/40", badge: "bg-white/20 text-white", gradient: "from-slate-300/15 via-slate-200/5 to-transparent" };
    default:
      return { glow: "hover:shadow-yellow-400/80", ring: "group-hover:ring-yellow-400/70", badge: "bg-yellow-400/20 text-yellow-300", gradient: "from-yellow-400/30 via-yellow-300/10 to-transparent" };
  }
};
...
function EditModal({ item, onClose, onSaved }) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg bg-[#0d1424]/95 border border-white/10 rounded-2xl shadow-2xl shadow-black/60 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="space-y-1.5">
            ...
            <Input value={form.nombre} onChange={(e) => handleChange("nombre", e.target.value)}
              className="!bg-white/[0.04] border-white/10 !text-white rounded-xl h-10 text-sm" />
          </div>

          <div className="space-y-1.5">
            <Label className="text-white/60 text-[11px] font-bold uppercase tracking-widest">Descripción</Label>
            <textarea value={form.descripcion} onChange={(e) => handleChange("descripcion", e.target.value)}
              rows={3} placeholder="Descripción de la carta..."
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label className="text-white/60 text-[11px] font-bold uppercase tracking-widest">Condición</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-white/60 text-[11px] font-bold uppercase tracking-widest">Estado</Label>
      ...
      <div className="w-full max-w-sm bg-[#0d1424]/95 border border-red-500/20 rounded-2xl shadow-2xl shadow-black/60 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-5">
          <p className="text-white/60 text-sm leading-relaxed">
            ¿Deseas desactivar la carta <span className="text-white font-semibold">"{item.nombre}"</span>?
          </p>
          <p className="text-white/35 text-xs mt-2">
            El estado cambiará a <span className="text-red-300 font-semibold">No disponible</span>. Podrás reactivarla con el toggle.
          </p>
        </div>
  ...
  return (
    <div className="flex justify-center px-4 py-4">
      <div className="
        ...
        ring-1 ring-black/50 bg-[#0a0f1e]
      ...
      ">
        {total > 0 ? (
          <>
            <img src={`${BASE_URL}/${imagenes[current].imagen}`} alt={`${nombre}-${current}`}
            ...
            {total > 1 && (
              <>
                <button onClick={prev} className="absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/60 hover:bg-black/90 backdrop-blur-sm flex items-center justify-center text-white/70 hover:text-white border border-white/15 hover:border-white/40 transition-all duration-150 z-10 opacity-0 group-hover:opacity-100">
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <button onClick={next} className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/60 hover:bg-black/90 backdrop-blur-sm flex items-center justify-center text-white/70 hover:text-white border border-white/15 hover:border-white/40 transition-all duration-150 z-10 opacity-0 group-hover:opacity-100">
...
export function ListCardCartas({ data, onRefresh }) {
  ...
  const handleSubasta = (item) => navigate("/subasta/create", { state: { carta: item } });
  ...
  const handleEditSaved      = () => { setEditItem(null);   onRefresh?.(); };
  const handleDeleteConfirmed = () => { setDeleteItem(null); onRefresh?.(); };
  ...
  return (
    <>
                ...
                <CardContent className="space-y-4 pt-4 text-white relative z-10">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className={`px-3 py-1 text-xs font-semibold rounded-full ${isInactive ? "bg-red-500/20 text-red-300" : "bg-green-400/20 text-green-300"}`}>
                    ...
                    <TooltipProvider>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button size="icon" onClick={() => setEditItem(item)} disabled={isInactive}
                        ...
                        <TooltipContent>{isInactive ? "Activa la carta para editar" : "Editar"}</TooltipContent>
                      </Tooltip>

                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button size="icon" onClick={() => setDeleteItem(item)} disabled={isInactive}
                        ...
                        <TooltipContent>{isInactive ? "Ya está desactivada" : "Desactivar"}</TooltipContent>
                      </Tooltip>

                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button size="icon" onClick={() => handleToggleEstado(item)}
                            ...
                            {isInactive ? <ToggleLeft className="w-3.5 h-3.5" /> : <ToggleRight className="w-3.5 h-3.5" />}
                          </Button>
                        </TooltipTrigger>
                        <TooltipContent>{isInactive ? "Activar carta" : "Desactivar carta"}</TooltipContent>
                      </Tooltip>

                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button size="icon" onClick={() => handleSubasta(item)} disabled={isInactive}
                        ...
                        <TooltipContent>{isInactive ? "Activa la carta para subastar" : "Crear subasta"}</TooltipContent>
                      </Tooltip>
      ...
      {editItem && (
        <EditModal item={editItem} onClose={() => setEditItem(null)} onSaved={handleEditSaved} />
      )}
      {deleteItem && (
        <DeleteModal item={deleteItem} onClose={() => setDeleteItem(null)} onConfirmed={handleDeleteConfirmed} />
      )}
    </>
  );
}
